...
##Covariates
Lets reload the brickman database
```{r reloading_data}
db = brickman_database() |>
...
Now we are going to make a pairs plot
```{r pairs}
pairs(present)
```

Now we are going to identify the most independent variables, and drop those that are least independent
...
We are now going to look at the model input data
```{r plot_pres_vs_bg}
model_input = read_model_input(scientificname = "Melanogrammus aeglefinus")
