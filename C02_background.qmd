...
obs = read_observations(scientificname = "Anarhichas lupus")
...
```{r save_model_input}
write_model_input(obsbkg, scientificname = "Anarhichas lupus")
```

Note that there is a function for reading model input files, too. 

```{r read_model_input}
x = read_model_input(scientificname = "Anarhichas lupus")
```


# Recap

We have prepared what we call "model inputs", in particular for *Anarhichas lupus*, by thinning observations and selecting background points.  There are lots of other approaches to creating the model input, but for the sake of learning we'll stick with this approach.  We developed a function that will produce our model inputs for a given month, and saved them to disk.
